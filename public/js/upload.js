...
if (getTagsBtn && modeConfig.aiEnabled) {
  getTagsBtn.addEventListener('click', async () => {
    ...
    const selectedModel = document.querySelector('input[name="aiModel"]:checked')?.value || 'clip';
    ...
    try {
      ...
      if (data.tags?.length) {
        ...
        const modelName = data.model?.toUpperCase() || 'CLIP';
        tagDisplay.innerHTML = `<div class="mb-2"><small class="text-muted">Generated by ${modelName} AI Model:</small></div>` +
